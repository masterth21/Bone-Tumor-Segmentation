...
def create_mask_from_json(json_path, img_shape):
    """
    Tạo mask từ JSON:
    - Nếu có vùng bệnh (Polygon/Rectangle): Vẽ màu trắng (255).
    - Nếu không có vùng bệnh (Ảnh không bệnh): Trả về mask đen hoàn toàn (0).
    """
    # Init black mask
    mask = np.zeros(img_shape[:2], dtype=np.uint8)
    ...
    try:
        ...
        if not has_polygon:
            for shape in shapes:
                if shape.get("shape_type") == "rectangle":
                    points = np.array(shape["points"], dtype=np.int32)
                    # Vẽ hộp chữ nhật đặc (fill) màu trắng
                    cv2.rectangle(mask, tuple(points[0]), tuple(points[1]), 255, -1)

    except Exception as e:
        # Nếu file lỗi, vẫn trả về mask đen để không làm gián đoạn quá trình train
        print(f"Warning: Could not process {json_path} ({e}). Returning black mask.")

    return mask


def process_data():
    # 1. Tạo cấu trúc thư mục processed
    for split in ["train", "val", "test"]:
        os.makedirs(os.path.join(PROCESSED_DIR, split, "images"), exist_ok=True)
        os.makedirs(os.path.join(PROCESSED_DIR, split, "masks"), exist_ok=True)

    # 2. Quét toàn bộ thư mục
    extensions = (".jpg", ".jpeg", ".png", ".JPG", ".JPEG", ".PNG")
    all_image_files = [f for f in os.listdir(RAW_IMG_DIR) if f.endswith(extensions)]

    # loại bỏ phần mở rộng
    valid_ids = sorted(list(set([os.path.splitext(f)[0] for f in all_image_files])))

    print(f"Tìm thấy tổng cộng {len(valid_ids)} ảnh trong thư mục raw.")

    # 3. Chia tập dữ liệu theo tỷ lệ 80:10:10
    train_ids, temp_ids = train_test_split(valid_ids, test_size=0.2, random_state=42)
    val_ids, test_ids = train_test_split(temp_ids, test_size=0.5, random_state=42)

    split_dict = {"train": train_ids, "val": val_ids, "test": test_ids}

    # 4. Xử lý từng tập dữ liệu
    for split, ids in split_dict.items():
        print(f"Đang xử lý tập {split}...")
        for img_id in tqdm(ids):
            # Tìm file ảnh thực tế (do có thể có nhiều định dạng khác nhau)
            img = None
            ...
            # Tạo Mask từ file JSON tương ứng
            # Nếu không có file JSON, hàm này sẽ trả về mask đen (ảnh không bệnh)
            json_path = os.path.join(RAW_ANN_DIR, f"{img_id}.json")
            mask = create_mask_from_json(json_path, (h, w))
            # Resize mask sử dụng INTER_NEAREST để giữ nguyên giá trị pixel 0 và 255
            mask_res = cv2.resize(
                mask, (IMG_SIZE, IMG_SIZE), interpolation=cv2.INTER_NEAREST
            )

            # Lưu ảnh và mask đã xử lý vào thư mục processed
            cv2.imwrite(
